...
stdio, calls the `path_between` tool with a `from`/`to` pin pair, and dumps
...
def main():
    ...
    parser.add_argument("frm", help="source pin, REFDES:PIN")
    parser.add_argument("to", help="destination pin, REFDES:PIN")
    ...
    try:
        ...
        init = server.request("initialize", {
            ...
            "capabilities": {},
            "clientInfo": {"name": "test-path-between", "version": "0.1.0"},
        })
        ...
        result = server.request("tools/call", {
            "name": "path_between",
            "arguments": {"from": args.frm, "to": args.to},
        })

        # 3. dump the tool output. Text tools return content blocks.
        print(f"=== path_between(from={args.frm}, to={args.to}) ===")
