...
LIBPARTS = {
    ...
    "MAGS":    ("Transformer", "H1102NL", "Ethernet magnetics", [
                    ...
                    ("3", "RD+", "unspecified"), ("4", "CT", "unspecified"),
                    ("6", "RD-", "unspecified"),
...
comp("R40", "R", "1 k", "/Ethernet/")     # space before the prefix (issue #18)
...
# its parent /Ethernet/ (issue #16).
wire("/Ethernet/Magnetics/CT", ("T1", "4"), ("C32", "1"))
...
wire("GND", ("H2", "1"), ("TP1", "1"), ("SW1", "2"),
     ...
     ("C30", "2"), ("C31", "2"), ("C32", "2"), ("R40", "2"), ("TP2", "1"))

# Every sheet in the schematic, in the order KiCad numbers them — the design
# header's list, which is authoritative where component sheetpaths are not:
# /Ethernet/Magnetics/ holds no parts, so this is the only record of it.
SHEETS = ["/", "/Power/", "/Sensor1/", "/Sensor2/", "/Sensor3/", "/Ethernet/",
          "/Ethernet/Magnetics/"]
...
def main():
    ...
    for number, sheet in enumerate(SHEETS, start=1):
        lines += [
            f"{IND * 2}(sheet",
            f'{IND * 3}(number "{number}")',
            f'{IND * 3}(name "{esc(sheet)}")',
            f'{IND * 3}(tstamps "{esc(sheet)}")',
            f"{IND * 2})",
        ]
    lines.append(f"{IND})")
